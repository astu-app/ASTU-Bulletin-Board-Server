...
public class AnnouncementServiceTest : DbInvolvingTestBase
{
    ...
    // Create
    [Fact]
    public void Create_RequesterIsNotAuthor_Throws()
    {
        var createAnnouncement = new CreateAnnouncement(
            content: "qwerty",
            authorId: MainUsergroupAdminId,
        ...
        var create = () => _announcementService.Create(UsualUser_1_Id, createAnnouncement);

        create.Should()
            .Throw<InvalidOperationException>()
            .WithMessage("Создать объявление может только его автор");
    }
    ...
    public void Create_NonExistingAuthorId_Throws()
    {
        ...
        var create = () => _announcementService.Create(createAnnouncement.AuthorId, createAnnouncement);
